...
public class EnumUtil {
	 ...
	 * NOTICE: The Enumeration Type must implement IMdekEnum !!!
	 * 
	 * @param <T>
	 * @param enumType
	...
	static public <T extends Enum<T>> T mapDatabaseToEnumConst(
		...
		for (int i = 0; i < myEnumConsts.length; i++) {
			if (!(myEnumConsts[i] instanceof IMdekEnum)) {
				break;
			}
			currDbValue = ((IMdekEnum) myEnumConsts[i]).getDbValue();
			if (currDbValue == null) {
				if (dbValue == null) {
	 ...
	 * Type. NOTICE: The Enumeration Type must implement IMdekEnum !!!
	 * 
	 * @param <T>
	 * @param enumType
	 *            e.g. WorkState.class
	...
	static public <T extends Enum<T>> Object[] getDbValues(Class<T> enumType) {
		...
		for (int i = 0; i < myEnumConsts.length; i++) {
			if (myEnumConsts[i] instanceof IMdekEnum) {
				ret[i] = ((IMdekEnum) myEnumConsts[i]).getDbValue();
			} else {
				ret[i] = null;
			}
		}
	...
	/**
	 * Get all database values of the Enumeration Constants of an Enumeration
	 * Type AS Strings ! NOTICE: The Enumeration Type must implement IMdekEnum
	 * !!!
	 * 
	 * @param <T>
	 * @param enumType
	...
	static public <T extends Enum<T>> String[] getDbValuesAsString(
			Class<T> enumType) {
		Object[] dbValues = EnumUtil.getDbValues(enumType);
