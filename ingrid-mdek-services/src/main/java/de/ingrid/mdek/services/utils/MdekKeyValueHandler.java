...
public class MdekKeyValueHandler {
	...
	public IEntity processKeyValue(IEntity bean) {
	    return processKeyValue(bean, null);
	}

	/** evaluate keys and set correct syslist values in bean according to bean type and dependent from data in object document ! */
	public IEntity processKeyValue(IEntity bean, IngridDocument objectDoc) {
		...
		if (T011ObjServ.class.isAssignableFrom(clazz)) {
			...
			processKeyValueObjectUse((ObjectUse) bean, objectDoc);
	...
	private IEntity processKeyValueObjectUse(ObjectUse bean, IngridDocument objDoc) {
		boolean isOpenData = "Y".equals(objDoc.getString(MdekKeys.IS_OPEN_DATA));
		MdekSysList syslist = isOpenData ? MdekSysList.OBJ_USE_LICENCE : MdekSysList.OBJ_USE;
