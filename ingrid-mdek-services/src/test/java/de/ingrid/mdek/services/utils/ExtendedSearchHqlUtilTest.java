...
public class ExtendedSearchHqlUtilTest {
    ...
    public void testCreateObjectExtendedSearchQuery() {
        ...
        searchParams.put(MdekKeys.RELATION, 0);
        ExtendedSearchHqlUtil.createObjectExtendedSearchQuery(searchParams);
        assertEquals("from ObjectNode oNode inner join oNode.t01ObjectWork obj inner join oNode.fullIndexObjs fidx where fidx.idxName = 'full' and (fidx.idxValue like '%hallo%' and fidx.idxValue like '%term1%' and fidx.idxValue like '%term2 term3%')", ExtendedSearchHqlUtil.createObjectExtendedSearchQuery(searchParams));
        searchParams.putInt(MdekKeys.RELATION, 1);
        assertEquals("from ObjectNode oNode inner join oNode.t01ObjectWork obj inner join oNode.fullIndexObjs fidx where fidx.idxName = 'full' and (fidx.idxValue like '%hallo%' or fidx.idxValue like '%term1%' or fidx.idxValue like '%term2 term3%')", ExtendedSearchHqlUtil.createObjectExtendedSearchQuery(searchParams));
        ...
        searchParams.put(MdekKeys.GEO_THESAURUS_RELATION, 0);
        ...
        searchParams.put(MdekKeys.CUSTOM_LOCATION, 12345);
